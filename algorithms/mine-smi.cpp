...
namespace gimlet {
  namespace itemsets {
    ...
    class SmoothedInfoTopK {
      using scorer_type = SmoothedInformation<partition_tree_parition_t, false, true>;
      using miner_type = IFPGrowth<scorer_type>;
      scorer_type scorer_;
      miner_type miner_;
    public:
      SmoothedInfoTopK(double alpha) : scorer_(alpha), miner_() {}
...
int main(int argc, char *argv[]) {
  ...
  try {
	...
	("alpha", po::value<double>(&alpha)->default_value(1.), "Laplace smoothing coefficient")
    ...
    SmoothedInfoTopK topKminer{alpha};
