...
namespace gimlet {
  namespace itemsets {
    ...
    class AdjustedInfoTopK {
      using scorer_type = AdjustedDependency<partition_tree_parition_t>;
      using miner_type = IFPGrowth<scorer_type>;
      scorer_type scorer_;
      miner_type miner_;
    public:
      AdjustedInfoTopK(double alpha) : scorer_(alpha), miner_() {}
...
int main(int argc, char *argv[]) {
  ...
  try {
	...
	("alpha", po::value<double>(&alpha)->default_value(0.95), "Probability value for Chi2 statistical test")
    ...
    AdjustedInfoTopK topKminer{1-alpha};
