...
namespace gimlet {
  ...
  namespace itemsets {
    ...
    template<typename Partition, typename Value, bool target>
    struct PartitionScore {
      using size_type = typename Partition::size_type;
      using value_t = Value;
      static const bool has_target = target;
    ...
    struct NoScore : public PartitionScore<Partition, void, false> {
      void min() {}
      void max() {}
    };

    template<typename Partition>
    class CountCollecter : public PartitionScore<Partition, std::vector<double>, false> {
    ...
    template<typename Partition>
    class Entropy : public PartitionScore<Partition, double, false> {
      using size_type = typename Partition::size_type;
    ...
    template<typename Partition>
    class SmoothedEntropy : public PartitionScore<Partition, double, false> {
    ...
    class SmoothedInformation : public PartitionScore<Partition, double, true> {
      using size_type = typename Partition::size_type;
	...
	return v1 < v2;
      ...
      SmoothedInformation(double alpha = 1) : alpha_(alpha), aloga_(xlogx(alpha)),
					      n_(), IXY_(), bound_(),
					      partitionY_(), HXa_(), HYx_(), HYgX_(), NX_(), NY_(), nys_() {}
	...
	return bound;
      ...
      std::pair<value_t, value_t> operator()(const Partition& partitionX) const {
    ...
    template<typename Partition>
    class ReliableFractionOfInformation : public PartitionScore<Partition, double, true> {
      using size_type = typename Partition::size_type;
    ...
    */
    template<typename Partition>
    class AdjustedDependency : public PartitionScore<Partition, double, true> {
      using size_type = typename Partition::size_type;
    ...
    */
    template<typename Partition>
    class SuzukiInfo : public PartitionScore<Partition, double, true> {
      using size_type = typename Partition::size_type;
